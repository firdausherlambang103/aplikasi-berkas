...
@section('content_header')
    <h1>Edit Registrasi Berkas: {{ $berkas->nomer_hak }}</h1>
@stop
            ...
            <div class="row">
                <div class="col-md-6">
                        ...
                        <select id="kode_klien_select" class="form-control">
                            ...
                            @foreach($klienTersedia as $klien)
                                <option value="{{ $klien->kode_klien }}" 
                                        data-id="{{ $klien->id }}"
                                        {{ $berkas->klien_id == $klien->id ? 'selected' : '' }}>
                                    {{ $klien->kode_klien }} ({{ $klien->nama_klien }})
                                </option>
                            @endforeach
                        </select>
                        <input type="hidden" name="klien_id" id="klien_id" value="{{ old('klien_id', $berkas->klien_id) }}">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="jenis_hak" class="form-control" required>
                            @foreach($jenisHakOptions as $option)
                                <option value="{{ $option }}" 
                                        {{ old('jenis_hak', $berkas->jenis_hak) == $option ? 'selected' : '' }}>
                                    {{ $option }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="kecamatan" class="form-control" 
                               value="{{ old('kecamatan', $berkas->kecamatan) }}" required>
                    </div>

                    <div class="form-group">
                        <label>Desa</label>
                        <input type="text" name="desa" class="form-control" 
                               value="{{ old('desa', $berkas->desa) }}" required>
                    </div>

                    <div class="form-group">
                        <label>Jenis Permohonan</label>
                        <input type="text" name="jenis_permohonan" class="form-control" 
                               value="{{ old('jenis_permohonan', $berkas->jenis_permohonan) }}" required>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="keterangan" class="form-control" rows="8">{{ old('keterangan', $berkas->keterangan) }}</textarea>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#kode_klien_select').on('change', function() {
            ...
            // Dapatkan klien_id dari atribut data pada opsi yang dipilih
            ...
            if (kode) {
                // Panggil API untuk mendapatkan nomer WA
                fetch(`/api/klien/get-by-kode/${kode}`)
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            $('#klien_id').val(''); // Kosongkan ID jika kode tidak valid
                        }
                    })
                    .catch(error => {
