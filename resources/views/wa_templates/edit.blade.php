...
@section('content_header')
    <h1>Edit Template: {{ $waTemplate->nama_template }}</h1>
@stop

@section('content')
<div class="card card-warning">
    <div class="card-header">
        <h3 class="card-title">Formulir Edit Template</h3>
    </div>
    ...
    <form action="{{ route('wa-templates.update', $waTemplate->id) }}" method="POST">
        @csrf
        ...
        <div class="card-body">
            <div class="form-group">
                ...
                <input type="text" name="nama_template" id="nama_template" class="form-control" 
                       placeholder="Contoh: Notifikasi Registrasi Selesai" 
                       value="{{ old('nama_template', $waTemplate->nama_template) }}" required>
            </div>
            
            <div class="form-group">
                <label for="template_text">Isi Template Pesan</label>
                <textarea name="template_text" id="template_text" class="form-control" rows="10" 
                          placeholder="Tulis pesan Anda di sini, gunakan placeholder di bawah..." required>{{ old('template_text', $waTemplate->template_text) }}</textarea>
            </div>
        ...
        <div class="card-footer">
            <button type="submit" class="btn btn-warning">Perbarui Template</button>
